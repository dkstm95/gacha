...
```bash
node src/investiq.mjs init
node src/investiq.mjs doctor
node src/investiq.mjs run discover "12개월 투자 후보를 최신 데이터로 찾아줘" --platform manual
```
...
The user-facing command is always `investiq`. The local `node src/investiq.mjs ...` form is only for repository development before linking or packaging.
