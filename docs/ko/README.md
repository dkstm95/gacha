...
Gacha는 단순한 전제에서 출발합니다. 투자 결과를 완벽하게 예측할 수는 없지만, 꼼꼼한 조사는 더 나은 확률을 만들 수 있습니다. Gacha는 불명확한 투자 질문을 현재 데이터 확인, 대안 비교, 리스크 점검, 정제된 리포트로 이어지는 구조화된 판단으로 바꾸는 데 도움을 줍니다.
...
Gacha는 투자 판단의 선명도가 서로 다른 사용자를 위해 설계되었습니다.

- 탐색: 아직 무엇에 어떻게 투자해야 할지 모르는 경우.
- 테마 선택: 투자하고 싶은 분야나 테마는 있지만, 구체적인 종목, ETF, 자산, 상품은 모르는 경우.
- 진입 계획: 살 대상은 정했지만, 언제 어떤 조건에서 매수할지 어려운 경우.
- 보유 관리: 이미 보유 중이고, 유지, 축소, 매도, 손절 기준이 필요한 경우.
- 포트폴리오 점검: 여러 포지션의 비중, 집중도, 중복 노출, 리스크를 확인하고 싶은 경우.

목표는 투자를 확실하게 만드는 것이 아닙니다. 더 빠르고 일관된 리서치를 통해 행동 가능한 판단 기준을 만드는 것입니다.
