...
func (m tuiModel) closeOrBackProfile() tuiModel {
	if m.profile == nil {
		return m
	}
	if m.profile.Mode == profileFlowOnboarding {
		if m.profile.Step > 0 {
			m.profile.Step--
			m.profile.Category = profileOnboardingSteps[m.profile.Step]
			m.profile.Cursor = profileInitialCursor(m.profile.Category, m.profile.Draft)
			m.view.SetContent(m.profile.render(m.lang, m.view.Width))
			m.view.GotoTop()
		}
		return m
	}
	if m.profile.Mode == profileFlowEdit {
		m.profile.Mode = profileFlowMenu
		m.view.SetContent(m.profile.render(m.lang, m.view.Width))
		m.view.GotoTop()
		return m
	}
